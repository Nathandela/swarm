...
internal class PushRegistrationRetry(
    ...
    private val attempt: (token: String) -> Boolean,
    ...
    fun submit(token: String) {
        ...
        scheduleAttempt(current, token, 0)
    }

    private fun scheduleAttempt(current: Long, token: String, index: Int) {
        schedule(backoffMillis[index]) {
            ...
            if (!accepted && generation.get() == current && index + 1 < backoffMillis.size) {
                scheduleAttempt(current, token, index + 1)
            }
        }
    }
}
