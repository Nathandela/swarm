 ...
 * @param online drives the mark and nothing else. It is TRUE only for the relay's own `online`;
 *  see [presenceDot], which argues why `unknown` is not `offline`.
...
fun machineRow(
    ...
    line.addView(
        presenceDot(context, online = online, description = presenceDescription).apply {
