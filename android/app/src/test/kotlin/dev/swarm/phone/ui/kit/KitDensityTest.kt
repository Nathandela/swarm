...
class KitDensityTest {
    ...
    fun `the components spend the whole pixels the platform would`() {
        val row = sessionRow(context, "quanthome/api", "claude", "Wants to run something", "working", lit = false, promoted = false)
    ...
    private fun row(group: String) = sessionRow(
        context, "quanthome/api", "claude", "Wants to run something", group, lit = false, promoted = false,
    )
}
