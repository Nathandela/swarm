...
//	    authorizes nothing. It is bidirectional so ADR-007 B59's decision cannot be half-changed.
...
var perUseSubjects = map[string]string{
	...
	// ADR-007 B59's BILL, and the row most likely to be the next instance if it is dropped.
...
//
// ADR-007 B59 decided AGAINST adding AUTH_DEVICE_CREDENTIAL to the content KEK and the per-use
...
//     biometric is B59 reversed by half, weakening the key with no visible change anywhere.
//
...
func TestPBSEC2_ThePromptAuthenticatorsMatchTheKeystoreSpec(t *testing.T) {
	...
	if keyAcceptsCredential != promptAcceptsCredential {
		t.Errorf("PB-SEC-2: the Keystore specs and the prompt disagree about device "+
			...
			"ADR-007 B59 decided this once, for both ends. Half of it is worse than either "+
			...
			"Change both, and amend B59 with the argument",
