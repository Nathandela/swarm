...
func TestOperationalExamplesNameTheCurrentRelease(t *testing.T) {
	for _, path := range []string{
		...
		if !strings.Contains(contents, "v0.13.20") {
			t.Errorf("%s does not name the current immutable release v0.13.20", path)
		}
		if strings.Contains(contents, "v0.13.19") {
			t.Errorf("%s still recommends consumed release v0.13.19", path)
		}
	}
}
