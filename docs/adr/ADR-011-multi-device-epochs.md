...
> **SCOPE MARKER (2026-08-14, ADR-018):** this ADR is the multi-**DEVICE** program — N devices paired to ONE machine. Multi-**MACHINE** — N machines paired to one phone — is `ADR-018-multi-machine-pairings.md`, ships first, and draws nothing from M1-M9. See the appended scope note at the end of this file.
...
1. **`SenderKeyID` is uniformly zero inbound.** Every phone seal leaves it at its zero value (`internal/phonecore/direction.go:57`, `internal/phonecore/input.go:44-47`, asserted by `internal/phonecore/direction_binding_test.go:140`). The gateway's inbound bucket is therefore always `{sender: 0, epoch}` (`internal/remotegw/mailbox_in.go:131`). Two devices would share one seq space and neither would be attributable past the shared high-water (`ADR-007:547-549`, "attributable past the shared seq high-water"; restated B1 at `:648-650`).
2. **One epoch `ContentKey`, shared by every paired device (D2).** It is what makes the machine→phone fan-out work — one ciphertext, N mailboxes, `recipient_key_id` outside the AAD (D6, `ADR-007:60`). On the phone→machine leg the same sharing means a paired device could seal a frame that authenticates as any other paired device's (`ADR-007:468-473`, the A7 "Cross-device input" residual).
...
**M9. The lease is bound to the sender id that opened it.** C6 asks for two things and the cheap-hardening sentence names both — stamp the sender id on take_control + input envelopes **and bind the lease to that sender id** (`ADR-007:472-474`, `:547-548`). M1 and M2 deliver the first only. Without the second, key separation buys attribution and not authorization: `routeInput` routes a frame by the session id sealed inside it and nothing else (`internal/remotegw/command_loop.go:547-554`), so device B's keystrokes would still ride device A's lease — correctly attributed and wrongly honoured. So `LeaseRouter.Begin` records the `SenderKeyID` of the envelope that carried the take_control and `Input` takes it too (`command_loop.go:57-59`: both signatures gain it), and an input frame whose sender id is not the holder's is **dropped**, fail-closed, in the same shape as the existing gap-drop. Phone-vs-phone contention falls out of the same record: a second device does not steal an open lease, it is refused until the holder releases or the lease expires. That is P-5's exclusivity applied *between phones*; Decision G's relaxation (`ADR-007:486-496`, "P-5's exclusivity is **relaxed for the personal single-owner model**") governs owner-tier-vs-phone for the single-owner model and is untouched — M9 does not re-exclude the owner tier, and it does not reopen the multi-user arbitration Decision G defers (see Sequencing).
...
- Decision G (`ADR-007:486-498`): concurrent owner-tier and phone control stays permitted. M9 arbitrates phone-vs-phone only.
...
**Shipping to a second *person's* hardware is a different gate, and this ADR is not it.** M9 arbitrates between one owner's own devices; multi-user exclusivity across tiers is deferred by Decision G (`ADR-007:486-496`) for the case where remote control is shared across distinct people, and nothing here decides it. That build needs that decision as well as these.
...
- Per-device inbound keys remove the shared-`ContentKey` forgery path on the inbound leg, and the M9 lease binding closes the other half the A7 residual names (`ADR-007:468-474`) — attribution *and* authorization — rather than declaring it moot.
...
**This ADR is the multi-DEVICE program: N devices paired to ONE machine.** The word is *device*, not
*phone*, in both this note and the scope marker under the header block, and deliberately so: the
peers M1-M9 govern are one owner's phones **and** tablets, and the multi-phone shorthand RC-D8 uses
(`playbook:78`, "multi-phone is later") names the same program from the product side without being
this ADR's own vocabulary. Everything in it — the
nonzero per-device `SenderKeyID`, the per-device inbound key from a machine-only `RootKey`, per-device
seq spaces and grant counters, the per-device reconcile authority, M9's sender-bound lease and M7's
admin tier — is the machine-side consequence of a second *device* of the same owner existing, and of
the boundary move that comes with it: "each paired device is in scope as an adversary of its peers"
(`:27`).

**Multi-MACHINE — N machines paired to one phone — is a different feature, is governed by ADR-018, and
ships first.** It draws on **none** of M1-M9. N machines is N separate instances of the relationship
v1 already ships (one machine, one phone, one epoch, one trusted holder), so there is no peer to
attribute against, no shared `ContentKey` to defend from a peer, no shared inbound bucket to fork and
no lease to arbitrate. ADR-018 is a phone/client-state change; the machine never learns that another
machine exists (`ADR-018:15`, `:29`).

**The machine-side single-device cap stays until this ADR is implemented as one coherent program.**
`Registry.AddSole`, `BeginPairing`'s fast-reject and `resolveGatewayParams`' `len(devices) != 1`
refusal are frozen by name in ADR-018 MM1 (`ADR-018:39-51`) and are not weakened, renamed or given a
sender-id argument by anything in the multi-machine work. This ADR's Sequencing ruling is unchanged:
M1-M9 "are not separable ... Partial adoption is worse than none" (`:94`), and the gate still
fires the moment a second device of the same owner is to be paired — which shipping ADR-018 does not
cause. The dependency runs one way: **ADR-018 depends on this ADR not moving.**

**Editorial, non-substantive:** the same pass renumbered this ADR's thirteen `ADR-007:<line>`
citations against the post-annotation ADR-007 — Wave R1's six in-place markers inserted twelve lines
into that file and shifted twelve of them, every one except D6's `recipient_key_id` clause at
`ADR-007:60`, which sits before the first insertion point — and added a short verbatim anchor quote beside the
load-bearing ones so the next insertion cannot silently invalidate them. The offset table and the
reason the rest of the repository was left un-renumbered are recorded at `ADR-007:8626` (B144's
line-number note). **No decision text moved**: every `M`-clause, invariant and consequence above says
exactly what it said on 2026-08-07, and this amendment's Proposed status covers the scope note only.
