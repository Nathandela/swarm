...
>
> **What changed the answer was the question, not the difficulty.** The paragraph above is correct that nothing characterizes the CLI's input region, and correct that deriving it from the grid would be the never-guess move IS-TOOL-2 forbids. Both halves of that reasoning stand. What it did not notice is that **the transaction does not need a provider-specific or grid-derived input revision.**
>
> The shim owns `ptyWriter`, the PTY master's only serialized writer, with a mutex it already takes on every write. Under that lock it can conservatively track input operations whose effects are characterized: character insertion/deletion, complete horizontal/home/end navigation, line kill and submit. It therefore answers the narrower question **is the owner's logical input line provably clean from the input stream the shim serialized?** It never infers from what the agent drew. Provider-dependent word/Meta bindings, history/completion, an in-progress paste and lone/incomplete escape sequences are unknown and therefore busy.
>
> `shimwire.TypeSubmit` carries one whole message. The shim requires that logical-line tracker to be clean, writes the text, waits `submitframe.Gap`, writes the carriage return — **all under one hold of that same lock** — or refuses having written nothing. Holding across the gap is the point: while it elapses nothing else may reach this PTY, which is exactly what stops the owner's keystroke, or a second phone send, landing between a message's two halves. `internal/skeleton/supervision.go:16` already records the same shape for the passive supervisor.
>
...
>
> **It errs safe by construction.** A known draft deleted back to empty becomes clean, fixing the physical-handset false-busy latch. A real draft still refuses, and any editor action whose effect depends on provider state remains unknown and busy. False refusal under uncertainty is still chosen over prompt corruption deliberately.
>
