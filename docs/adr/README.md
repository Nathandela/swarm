...
1. Next sequential number (the next FREE one is ADR-024: 013 was minted for the Mirror capture architecture, `docs/specifications/mirror-program.md` M3.1 has reserved 014 for paged interaction history and that reservation still stands, 015-018 were allocated together by Wave R1 for the four playbook §3 amendments, 019 was minted for boundary-aware detach recognition, 020 for the unattended-restart environment decision, 021 for the Slate palette and breathing scale, 022 for live session name adoption, and 023 for ContextGuard auto-compaction)
