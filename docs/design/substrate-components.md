...
| 15 | Settings row | `.setrow` | `--p-card` | 1 dp `--p-hair` | `--p-card-r` 9 (mock 12) | padding `space_12` x `space_14`; gap `space_10` | label `Title.Row` / `--p-ink`; sublabel `Body.Secondary` / `--p-ink2` (mock 11.5 -> 12) | `--p-card-fx` key-light. Trailing control is row 4, or status text `Label.CardHead` / **`--p-hero`** — not `--p-ok`: "active" is a liveness statement, which is what hero means, and `--p-ok` now carries `ReadyForReview`. The whole row is one >=48 dp target when it carries a toggle |
...
| Machine row | `.mrow` | **Takes `sessionRow` unchanged.** Substrate draws no machines screen, and the mock's row is `.prow`'s shape wearing different words: a dot, a bold name, a mono identifier in `--p-ink3`, and a secondary line under them. `.mrow .eid` (mono 10 `#636366`) and `.prow .ag` (mono 10 `--p-ink3`) are the SAME cell. So the derivation is a reuse, not a component: machine name -> `project`, `endpoint <id>` -> `agent`, the session-count/relay line -> `need`, presence -> `group`. The mock's own geometry (radius 14, padding 13/14) is discarded in favour of `.prow`'s `--p-card-r` 9 and `space_10` x `space_12`, per §2 — two card rows one screen apart must not be two radii |
| Machine presence dot | `.mrow .dot.g-done` / `.g-off` | `--p-ok` online, `--p-ink3` offline, 7 dp, no glow — already stated in the status-dots row above. **What is NOT settled: `statusDot` takes a `status.Group` and the binding gate refuses any key that is not one** (`TestPBDS7_TheStatusDotBindingIsTheCheckedInMapping` fails on a bound name absent from `android/group-tokens.tsv`, deliberately — the phone renders the server's Group verbatim and never invents one). Machine presence is not a Group and must not be smuggled in as one. *Recommendation:* a separate `presenceDot` factory over the same drawable and the same 7 dp, bound to `--p-ok` / `--p-ink3` directly, so the Group fence stays strict. Do not widen the Group table |
| Activity row | `.arow` | Card geometry from `.prow` (`--p-card`, 1 dp `--p-hair`, `--p-card-r`, padding `space_10` x `space_12`), gap `space_10`. Timestamp `Mono.Meta` 10.5/500 `--p-ink3`; body `Body.Message` `--p-ink2` with its emphasised span in `--p-ink`, which is `.prow .ln` / `.ln b` unchanged. **The mock's fixed 52 px timestamp gutter is dropped and nothing replaces it**: `HH:MM` in a monospace face is the same width every time by construction, so the column aligns without a constant — and 52 is not on the spacing scale, so keeping it would have meant either a tenth literal exemption or a fabricated step |
| Remote-access state | `.kills` | `settingsRow`, label `Remote access`, sublabel naming the daemon-side switch, **`trailing = null`**. **The mock's toggle does not ship, and this is not a styling decision.** `App.KillSwitchEngaged` is read-only by design — `protocol/server.go handleRemoteSetControl` refuses the remote tier before the backend is consulted, on the stated grounds that *a remote device must never re-enable a switch its owner turned off*. A toggle here would be a control that cannot act, and the mock's red `--p-err` border goes with it: a danger border over a read-only status line advertises an action the screen does not have. The phone's destructive action is Revoke, one row below, and it is real |
| Paired-device row | `.devrow` | `settingsRow`, label = device name, sublabel = `key <fingerprint> · this device` in `Body.Secondary` (the fingerprint is `Mono.Agent` per §7), `trailing = ctaButton(kind = DENY)` reading `Revoke`. The mock's bespoke `.rev` button — its own border, radius 8, padding 7/11 — is discarded: it is `.a2-no` with different numbers, and §2's reuse rule is what stops a second denial control existing |
