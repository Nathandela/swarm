...
>
> **The measured contrast**, computed over Slate's ladder *(AMENDED 2026-08-28, ADR-021 D1: the
> figures are `android/gate/slate_contrast_test.go`'s own `wcagRatio`; over Obsidian's ladder
> `--p-hero` was `#c9a876` at 8.74 / 8.22 / 7.69:1)*: `--p-hero` `#8eb4e6` is **9.03:1** on
> `--p-bg`, **8.30:1** on `--p-card` and **7.35:1** on `--p-elev` — every one of them well
...
**AMENDED 2026-08-26 (owner ruling on the conversation surface, docs/specifications/chat-surface-plan.md).**
Rows 26 and up are NOT PB-DS-7 components and are not counted in the 25. They are the parts the
chat surface needs and Substrate never drew, because Substrate drew a triage inbox and a terminal
peek and this product now draws a conversation. They obey the same rules as every row above: tokens
only, an exception named where one is spent, and the reasoning where the value is chosen.
...
| 7 | SAS display | `.sas`; mobile direction frame 03 / Signal Field | path transparent → `--p-hero` → transparent behind six `--p-card` beacons | beacon border 1 dp `--p-hair`; path 1 dp | beacon `--p-card-r` | six equal peers, weight 1; beacon 48 high with adaptive equal width; gap `space_8`; margin `space_18` top / `space_8` bottom; path inset `space_24` | `Display.SAS` 34 sp — **the one style this document adds to PB-DS-2's 18** (§5). Ink: **exception** — emoji glyphs carry the platform emoji font's own colour, no token applies | Six protocol symbols remain in source order. One accessibility node reads "Verification symbols, in order: …"; tiles and static path are decorative children. No motion. Cross-vendor glyph divergence is an open question (§8.4) |
...
| 9 | Composer | `.composer` | bar `--p-tabbg` (`#E008090A`) + 16 dp blur (mock 18; one shared blur constant with the tab bar); field `--p-well` (`#FF060708`) | bar: 1 dp `--p-hair` top rule; field: 1 dp `--p-hair` | field `--p-card-r` 9 (mock: an 18 pill) | bar padding `space_8` x `space_14`, gap `space_8`, bottom `tabbar_height` 74 (mock 82), height `composer_height` 52; field padding `space_8` x `space_14`, visual height 36, touch target 48; glyphs 26, stroke 1.8; action-box 40 (AMENDED 2026-08-28, phone refit W3.2, owner ruling: the bar carries ONE control that sends, or stops while the agent works and the field is empty; the control's box is that square, kept on the row's 48 dp touch floor, its glyph the kit's ink; disabled: ink3, the ink every dead control shares, because the surface holds it disabled offline and while a send crosses and says so through the drawable state alone) | field `Body.Message` / `--p-ink`; placeholder `--p-ink2` — **not `--p-ink3`, which is 3.50:1 on the well and fails the 4.5:1 text floor**; ink2 gives 6.21:1 | Voice glyph `--p-ink2`; stop glyph `--p-err`. Both 48 dp targets, both keep the mock's content descriptions. The field is a *well*, inverting the mock's lighter-than-its-bar field: `--p-well` is the token for recessed input, and against a translucent bar a `--p-card` field would barely separate |
...
| 13 | Paired-device row | `.devrow` | `--p-bg` (the grouped Settings ground; Slate `.trow`) | 1 dp `--p-hair` on the bottom edge only | none | padding `space_14` x `space_18`; gap `space_10`; no outer tile inset | name `Title.Row` / `--p-ink`; fingerprint `Mono.Agent` / `--p-ink3` | Reuses row 15's flat ruled-row composition. **Revoke takes the `.a2-no` treatment at chip metrics**: fill `--p-err` at 13% as a transparent tint, ink `--p-err`, no border, `--p-chip-r` 8, padding `space_8` x `space_10`, `Label.Chip`, 48 dp target. |
| 14 | Activity row | `.arow` | `--p-card` | 1 dp `--p-hair` | `--p-card-r` 9 (mock 12) | padding `space_12` x `space_16` *(AMENDED 2026-08-27, ADR-021 D2: the Slate maquette's `.slab { padding: 12px 16px }`; was `space_10` x `space_12`, the session row's, mock 11/13)*; gap `space_10` | timestamp `Mono.Meta` / `--p-ink3`; body `Body.Message` / `--p-ink`; inline emphasis `Mono.InlineStrong` / `--p-ink` | `--p-card-fx` key-light. **The timestamp column is wrap-content, not the mock's fixed 52 dp** — a fixed column clips at the 1.3x font scale PB-DS-12 requires |
| 15 | Settings row | `.setrow` / Slate `.trow` | `--p-bg` (the screen ground) | 1 dp `--p-hair` on the bottom edge only | none | padding `space_14` x `space_18` (Slate's 13 x 18 mapped onto the approved scale); gap `space_10`; consecutive rows have no outer margin or card gap | label `Title.Row` / `--p-ink`; sublabel `Body.Secondary` / `--p-ink2` | Flat grouped list row: no card fill, rounded outline or `--p-card-fx`. Trailing control is row 4; the whole row is one >=48 dp target when it carries a toggle. **AMENDED 2026-08-29 (agents-tracker-2d22.3):** the later per-row card derivation is superseded by the signed Slate Settings composition and handset feedback; the status-text trailing form remains retired. |
...
| 26 | Message bubble | — (new) | `--p-elev` (`#FF1F1A13`) — the raised surface, so the reader's own words sit ABOVE the ground the agent's prose is written on. The agent draws no bubble at all: two bubbles is a chat between two strangers, and one of these two is the reader | none, in the settled state — a fill IS the differentiation, and a border on top of it is a second statement of the same thing | `--p-card-r` 14 on all four corners. **NO TAIL, and that is a decision rather than an omission.** The drawing this came from gave the bubble a small bottom-trailing corner, which is the messaging idiom — but the radius ladder in this skin is 14 / 18 / 10 / 8 and has no step small enough to read as a tail. Spending `--p-chip-r` 8 there would be a corner that looks slightly wrong rather than deliberately anchored, and inventing a 4 is inventing a step. The alignment and the fill already say who spoke; a tail would be a third statement of it | padding `space_12` x `space_16` *(AMENDED 2026-08-27, ADR-021 D2: the Slate maquette's `.slab { padding: 12px 16px }`; was `space_8` x `space_12`)*; gap between consecutive bubbles `space_14` *(the slab's `margin-bottom` 14px; was `space_8`)*; **max width 82% of the reading column** — a PROPORTION and not a step, deliberately: a dp maximum clips at the 1.3x font scale PB-DS-12 requires, and what the rule is actually about is leaving the ground visible down one side so the column still reads as a conversation | `Body.Message` / `--p-ink`. A `/command` takes `Mono.Code` / `--p-ink` — the reader typed a machine word and the type says so | **Three states, and two of them are claims about delivery.** SETTLED is the fill alone. PENDING is the same fill with the ink at `--p-ink2` and a 1 dp `--p-hair` border: the message exists and the agent has not echoed it, which is the only honest reading until it does (owner ruling R6). REFUSED keeps the ink and takes a 1 dp `color-mix(--p-err 36%, --p-hair)` border — row 12's own recipe, so a refusal looks like every other refusal in this app. No motion: a bubble that animated in would be a delivery animation over a fact we do not have |
| 27 | Conversation header | — (new) | `--p-bg` — the ground, so the header does not read as a card floating over the conversation; it IS the top of the screen | 1 dp `--p-hair` BOTTOM rule only, which is the tab bar's own construction turned upside down: a fill and a single hairline across the full width. Not a box | none — a full-bleed strip has no corners to round | padding `space_10` x `space_18`, matching `navHeaderDrill` so the back chevron sits where it sits on every other drill-down; the subtitle `space_2` under the title; the overflow a 48 dp target at the trailing edge | title `Title.Row` / `--p-ink`, ellipsized at the end — a session name is a project name and the front of it is what distinguishes two; subtitle `Mono.Meta` / `--p-ink3`, because the machine name and the state are MACHINE FACTS and this skin puts those in the mono face; the overflow glyph `--p-ink2` | **The state word comes with a dot, and the dot is `statusDot` unchanged** — the roster's own Group vocabulary, with its own colours and its own glow rule, so a session reads the same here as it does on the row the reader tapped to get here. Reusing `presenceDot` instead was considered and rejected: that component answers ONLINE / OFFLINE / UNKNOWN, which is a fact about a MACHINE, and what this header reports is what a SESSION is doing. **NOT `navHeaderDrill` WITH EXTRAS**: that component is a screen title and a way back, and this is an identity — who you are talking to, on which machine, doing what right now — which is why it carries a subtitle at all. Its title also takes the ROW rung rather than the display rung, deliberately reversing ADR-012 phase 2 P4 for this one header: P4's ruling is that a screen is a screen and depth is the chevron's job, and a conversation is not a screen the reader navigated TO so much as a thing they are inside |
| 28 | Conversation menu | — (new) | `--p-elev` — **row 1's toast surface, unchanged**. A floating block over the conversation is the same kind of object a toast is: opaque, one ladder step above the ground, with nothing reading through it. §2's reuse rule, and a fourth surface recipe here would differ from row 1's in no cell | 1 dp `--p-hair` on the block; 1 dp `--p-hair` BETWEEN rows (`div + div`), and that is a rule rather than a gap on purpose — a gap says these are three items in a list, a rule says these are three separate acts and the last one ends a session | `--p-card-r` 14, row 1's | block padding `space_4` (the smallest step: what holds a row off the edge is the ROW's padding, and this is what stops a row's focus ring sitting on the block's hairline); row padding `space_8` x `space_10`; hugging width, anchored under the overflow — a full-bleed block here would be a sheet; **48 dp target** on every row AND on the overflow control, which is one floor and two controls (see the overflow's own cell). The drawn row is under 34 dp, and the row under a mis-aimed tap on `Load earlier messages` is `Kill session` | row label `Body.Message` / `--p-ink`; destructive row `Body.Message` / `--p-err` — the notice line's pair of inks and the same claim: what changes is who is speaking, not how loudly, so the size and the weight are identical in both. Overflow glyph U+22EE, `Title.Row` / `--p-ink2` (row 27's own cell) | `--p-card-fx` key-light, row 1's, for row 1's reason: the block floats. **Two things are never here, and both are refusals.** No TERMINAL VIEW — ADR-017:60-65 forbids a raw-terminal route on a session with a structured record, so the row would be a door onto a room that is not there. No REPAIR — the tear has a position and the repair is drawn at it (row 29); two affordances for one live-only operation are two pending states competing over which is in flight. **The choices are the caller's**, because which of them exist is a fact about a session (an ended session has no kill) and a menu holding its own list would draw them anyway and refuse them on tap. **The glyph is the character and not an asset**, which is the opposite of §4's chevron and decided by the same rule: that row could give the chevron a path because `swarm_nav_back.xml` holds one, and there is no overflow asset in this app — drawing one would be inventing a design value in code. Kill keeps the confirmation it ships today, word for word, drawn by the screen after this block closes: R2 moves the control and does not touch its question |
| 29 | Gap divider | — (new) | none (the ground shows) | 1 dp `Kit.errorBorder` = `color-mix(in srgb, --p-err 36%, --p-hair)` — **row 12's own derivation spent a third time, not a fourth hand mix.** The drawing writes the rule as `rgba(--p-err, 0.32)`, and PB-TOK-7 forbids typing an alpha that is a function of a token, so the faithful-looking implementation is the one the token regime refuses. What is already declared and already checked by `internal/design.Derivations()` is a hairline warmed toward the error ink, which is the same gesture; the arithmetic is 0.36 x `--p-err` + 0.64 x `--p-hair` = `#FF6B3F36`, against the drawing's 0.32 alpha over `--p-bg` = `#FF4F2925` — one ladder step brighter, and shared with two components rather than mixed a third time | none — a rule has no corners | rule height 1 dp, **`Kit.dpPx` and not `Kit.dp`**: this is a laid-out height rather than a stroke, which is where the distinction is easiest to lose, and at density 2.625 the unrounded value lays out 2 px against the 3 px the header's own rule paints on the same screen. gap `space_8` between the rule and the word (drawing 9, row 11's own rounding); both rules weighted equally so the word sits at the centre of the column whatever it says; no margin — the air belongs to the composing column (§4's notice line cell) — and the component's own **48 dp target** IS the air at a tear | `Body.Secondary` / `--p-err` — §4's notice line in its ERROR voice, unchanged | **It is `notice(ERROR)` minus the paragraph.** Today a proven gap draws `SessionDetailPanel.kt:248-249`'s sentence and two full-width buttons ABOVE the conversation: a notice with no position, standing where the reader is reading, on a screen with roughly 150 dp left for messages. What changed is how much of the screen the statement may take, not who is making it. **It is not the sync notice and may never borrow its drawing** — a stale stream has no position, so it cannot claim one, and that notice stays the full-width sentence it already is. **The repair is a word in the label and the whole line is the control**, which is why the floor is spent on the line: row 22 states the general case, an inline span cannot carry a target. The line is drawn only where a tear is PROVEN and positioned; a divider drawn on suspicion is a claim about the record made from what this phone happens to hold |
| 30 | Earlier chip | — (new) | `--p-card` — **row 10's floating `.chip`, unchanged**, which is also what `syncPill` spends. §2's reuse rule: a second recipe would differ from row 10's in no cell | 1 dp `--p-hair`, row 10's | `--p-chip-r` 8, row 10's. **The drawing's `border-radius: 999px` is the CSS idiom for a pill and not a fifth radius**: this ladder is 14 / 18 / 10 / 8, row 3's badge gets a pill by degeneracy (`2 x 8 >= 16`) and row 4's toggle takes a NAMED exception for one, and a control carrying a 48 dp floor would need a 24 to read as a capsule. `syncPill` settled this already and is drawn the same way | padding `space_8` x `space_12`, row 10's; hugging width; **48 dp target**, row 10's own `targets >=48` arriving with the shape rather than being decided here — a pill sized to one short line of copy is the control that measures short by construction, and it is a minimum and not a size, so the drawing is unchanged wherever the metrics already clear it. No gravity and no margin: the drawing centres it over the first message, and centring is layout, which `android/gate/s24_screens_test.go` leaves to the screen | `Label.Chip` / `--p-ink2`, row 10's | **A chip and not a button, and that is the whole row.** The obvious implementation is the full-width tertiary CTA this app already has (`ctaButton(kind = MORE)`), and it is refused because the defect this surface exists to fix is three full-width buttons standing above a conversation — a fourth at the top of the list is the same mistake at the other end. **The honesty rule is the new part and it is the caller's**: the chip is drawn only while the machine says there is genuinely something older to fetch, and when it says nothing older is retained the chip is ABSENT rather than present and dead. A component never sees that fact, so the rule is recorded here and lands on the screen that composes it — `navHeaderDrill`'s nullable back control is the same arrangement |
| 31 | File change row | — (new) | `--p-card` — **row 14's activity row, verbatim**, because this row sits AMONG tool rows in the same stream: a different fill would make it read as a different KIND of object when it is the same kind carrying different cells | 1 dp `--p-hair`, row 14's | `--p-card-r` 14, row 14's | padding `space_12` x `space_16` and gap `space_10`, all row 14's — including ADR-021 D2 (2026-08-27), which widened the activity row to the Slate slab's pair: this row kept `space_10` x `space_12` for one wave pending its own ruling, and W8.2 (2026-08-28) ruled that it follows, because in the stream the tighter box read as a different kind of object, which is this row's own argument; the path cell takes the slack so the verb and the counts keep their own widths; cells share a baseline (row 14's arrangement) so the sans verb and the mono path sit on one line, with `CENTER_VERTICAL` placing that line inside the box; **48 dp target** — the row opens a diff on its own screen, so it is a control and not a record, which is the one thing separating it from the activity row it is otherwise drawn as (row 14 states no floor because nothing there is tappable) | verb `Body.Message` / `--p-ink` — the wire's own word, and the sentence of this row rather than a qualifier on one; path `Mono.Meta` / `--p-ink3` — a path is a machine fact and this skin puts machine facts in the machine's face (row 27's subtitle makes the same call); counts `Mono.Meta` / `--p-ink3` | `--p-card-fx` key-light, row 14's. **Owner ruling R9**, and what it must not become is what ships today: `TranscriptPanel.kt:465` draws a unified diff inline and unconditionally, so a twelve-file refactor costs a screen per file on the one surface whose purpose is continuous reading. The diff is moved, not deleted — it opens on a screen that can give it room to scroll sideways. **The path ellipsizes at the MIDDLE and is the only identity in this kit that does**: `Kit.identityCell`'s rule is that a name is distinguished by its front, true of a project, a machine and a session and false of a path, whose distinguishing half is its last segment. **The counts are one wire-authored string in one ink** — the drawing tints `+N` `--p-ok` and `-M` `--p-err`, and doing that means splitting the machine's own words to decide which half is which, the parse IS-TOOL-1 refuses one hop earlier; the sign carries the direction and the sign is the wire's. **Never grouped**: a count of files is not a record of what changed |
| 32 | Decision pill | — (new) | `--p-hero` — row 3's badge recipe, and the champagne is what "you" looks like in this skin (ADR-009 D6); a question the agent is blocked on is the strongest reading of that there is | none — a saturated fill carries the state on its own, `.chip.on`'s rule | `--p-chip-r` 8, row 3's, and the pill is a consequence rather than a fifth radius (row 30's cell argues it) | padding `space_6` x `space_14`; hugging width; **48 dp target** — it is the one control on this screen a person reaches for while the agent is working, which is when a thumb is least careful, and it is a minimum and not a size on row 13's terms. No gravity and no margin, for row 30's reason | `Label.Chip` / `--p-hero-ink` — a saturated fill takes the ground ink and not the linen, which is row 3's pairing | **No `--p-cta-fx` bloom, and that is the standing rule rather than restraint**: nothing glows unless it is alive, and owner ruling R8 narrowed the one glow this app has to the single session that needs you — a second champagne halo on the same screen dilutes the first, which is the argument R8 made when it took the glow off the Working dot. **What makes it legitimate is the condition, not the paint.** It is drawn ONLY while an unanswered decision is off screen, and auto-scroll to the newest message is suppressed while one is unanswered; both halves belong to the screen, because a component never knows where it is. What the component can do is refuse to be anything else — no count, no dismissal, no second state. **The arrow the drawing puts after the words is not here**: the copy table records `Needs your answer`, and a string not on that sheet is not on the screen. Where the decision is gets said by the scroll the tap performs |
...
| Scanner reticle | — (the mock draws no scanner); mobile direction screen 02 / Signal Field | The framing square over the camera preview: where to point the phone and how close to hold it. Ink: brackets `--p-hero` (`#FF8EB4E6`), the token `android/design-tokens.tsv` glosses as brand *and live*, and a viewfinder is the most literal live surface this app has. **`--p-ink` is rejected on scannability, not on meaning** — row 6 draws the symbol this frames as a `#FFFFFFFF` tile, so a white reticle over a code held to fill it is a white line on white. The four status tokens are rejected for the brackets because they mean state, and `--p-ink2` / `--p-ink3` are 5.87 and 3.30:1 against a *surface*, before a photograph is behind them at all. Geometry: frame 180, which is the size row 6 draws this same symbol at, centred and clamped to the preview when the preview is the smaller of the two; stroke 2, `.prow.attention`'s rail weight rather than the 1 dp hairline, because what is behind it is a moving image and not a flat surface; arm 24, so each corner paints `space_24` along both its edges and the middle of every edge stays open — a closed rectangle is a border, and what says *aim here* is four corners. Radius `--p-card-r` 9: row 6's, because this frames row 6's tile. A static 1 dp `--p-work` trajectory crosses the centre, inset `space_24` inside the frame and fading through work-teal at alpha zero before the brackets: unlike the bracket, this line explicitly reports that the camera is actively working, without adding a fourth motion. **It is not a control.** No fill, no click, no focus, and no touch target of its own — it is a `Drawable` foreground rather than a view precisely so it cannot acquire one, since a view over a live preview that could take a tap is what PB-SEC-12 clause 1 exists for. **And it does not move**: ADR-007 B134 keeps three motions and this is not among them. A decode-confirmation flash was specified with it and is not built — §8.9 |
| Signal field welcome | mobile direction frame 01 / Signal Field | The checked-in atmospheric swarm raster, with no landscape and no moon, floating directly on `--p-bg`. Geometry: mark 150 high at match width / `FIT_CENTER`; alpha 0.72. The mark is decorative and hidden from accessibility because the centred title directly below carries its meaning. It appears only for `FIRST_RUN`: revoked and repair-required screens keep their factual recovery composition and commands. No surface, border, animation, or new colour. |
...
same six symbols*, and it is comparing Noto Color Emoji on Android against Apple Color Emoji on
