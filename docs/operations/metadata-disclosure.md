...
claim is withdrawn."* D11 also forbids claiming **less** exposure than exists, which is why two
entries below are corrections to earlier, more flattering statements rather than new findings.
...
The provider observes **the token, the timing, and the size** — and PB-PUSH-3 records in as many
words that this claim was **false for the obvious implementation**, which is why it is enforced
...
Both are pinned by test (`TestPBPUSH3_WakeEnvelopeIsExactlyTheFixedContentFreeSize`,
`_WakeHeaderCarriesNoStableEndpointIdentifiers`, `_WakePlaintextIsEmptyAndNamesNothing`).
...
| **Timing** | Every wake, to the second. Transitions are coalesced per session, so this is a lower bound on activity, not a transcript. |
| **Size** | Constant 78 bytes. Carries no information. |
...
### 2b. Nothing here has ever run against Google
...
- **The gateway does not apply that policy.** `cmd/swarm-remote` dials with `relay.Dial`, not
  `relay.DialSecure`, so an operator who points `relay_url` at a non-loopback `ws://` address gets
  a cleartext hop with nothing refusing it. Keep the gateway on loopback
  (`docs/operations/relay-runbook.md` §0).
- **The mobile façade does the same.** `mobile/relay.go`'s `App.dial` calls `relay.Dial`, and
  pairing calls `relay.DialRaw`. Neither carries a `relay.Security`, so the pin, the cleartext
  refusal and the redirect re-check are not applied on the handset's connection either. This is
  recorded as an open finding, not a property; see the S20 evidence file.
