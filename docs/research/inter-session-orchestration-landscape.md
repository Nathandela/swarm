...
Research input to ADR-010. Question: from inside a running coding-CLI agent session,
...
to ADR-010's spawn/--handoff/--delegate/send surface. agmsg demonstrates that "inject
...
- Agent-authored handoff documents seeded as the new session's first prompt — the
  ...
  session-handoff skill. Vendor-neutral by construction. Chosen for ADR-010.
