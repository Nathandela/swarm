...
- **CG.5 Rollout**: Codex observation is version-bound to the running app-server initialize response (0.150.x/0.151.x characterized). Both ADR-023 concurrency gates recorded adversarial evidence on 2026-09-01 — both NEGATIVE — so production dispatch (ADR-023 amendment 1) runs under daemon-side enforcement with an exact live-gated version allowlist; non-allowlisted characterized versions are observe-only. Other harnesses remain unavailable rather than using terminal-text fallbacks.
...
- E7.5 Launch form: free-text cwd with `~` expansion; a non-directory is refused inline, while a missing cwd first offers edit-distance-ranked sibling suggestions and requires a second consecutive Enter to create the unchanged path (L-3); agent picker greys both not-installed AND out-of-supported-version-range CLIs with install/upgrade hint (L-2); options rendered from declarative adapter schema; initial prompt field; worktree toggle placeholder.
...
- E8.2 Detach on `Ctrl+q` (configurable; ADR-006), accepting legacy control-byte and Kitty CSI-u press encodings across split/coalesced reads while bracketed paste remains byte-exact. PTY-based integration test verifies termios restoration after: normal detach, Go panic, SIGINT, SIGTERM, SIGHUP. SIGKILL restoration is not claimed (impossible without a wrapper process — accepted limitation, documented).
