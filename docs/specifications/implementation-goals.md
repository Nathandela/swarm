...
- E9.2 Adapter statelessness: conformance suite exercises adapters through the interface only; review checklist greps adapter packages for `os.Open`/`os.Create`/`os.MkdirAll`/`net.Listen`/`net.Dial` outside tests — zero hits, result recorded in the epic evidence file.
