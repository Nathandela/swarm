...
| M1.3 | Terminal-side attribution: add the `PermissionDenied` capture row; verify the allow path resolution among the five daemon paths | `internal/adapter/claude/claude.go:78-89`, `approval.go:212-240` | `approval_resolved` carries `by: owner` vs `by: phone` correctly |
