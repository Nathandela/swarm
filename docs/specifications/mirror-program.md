...
| M4.2 | Codex `InteractionSource`: `userMessage` -> `user_message`; `agentMessage` deltas -> `agent_message` increments folded by ref, **batched 200 ms at the adapter from day one** (the 8 appends/s gateway ceiling is real); `commandExecution` -> `tool_run` kind `execute` with args + results; turn lifecycle -> `turn_id` + status | first true token streaming through M2.7's renderer. **The 200 ms fold ships at the PRODUCER EDGE (`internal/skeleton/backend.go`), not inside the adapter**: ADR-013 §R7.4 records the deviation and its reason (an adapter that batched would have to hold state across calls, which ADR-010's purity rule forbids). **The fold is held across frames that can shape no item** (round 4): measured against the RECORDED frame mix rather than a pure delta stream, flushing on every non-delta frame produced 10 offered records/s against a machine budget of 8 |
