...
| PB-TOK-5 | **Every colour token reaches the app, not three of them.** PB-TOK-1 built a bidirectional join and then ran three rows through it (`--p-bg`, `--p-ink`, `--p-ink2`); the remaining 13 colours have no Android representation, so 13 of 16 tokens are pinned against a design source that no screen can consume. The join is correct and under-fed. *(`--p-cta-bg` and `--p-cta-ink` are value-aliases of `--p-hero`/`--p-hero-ink`; the mapping must still name them separately, because a future skin can break the alias and a join that silently deduplicates would not notice.)* | All 16 colour tokens have a row in `android/design-tokens.tsv` and a `<color>` in `colors.xml`; the existing bidirectional gate (unmapped `<color>` fails, divergent value fails) is unchanged and now covers 16 rows. |
