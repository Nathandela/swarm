...
| PB-STATE-4 | Writes are crash-atomic; corruption fails **closed**. **Rollback needs a named trust anchor**: AEAD and atomic writes detect corruption, not rollback — a valid older blob sealed by the same Keystore key stays valid, and KeyMint rollback-resistance protects key blobs, not arbitrary app state. **The v1 anchor is chosen here, not left to the implementer** (v3's "or an explicitly narrowed threat" let an implementation decline rollback protection entirely): **remote high-water reconciliation on reconnect** — the gateway's durable inbound high-water (PB-GW-1) is the authority, and on every reconnect the phone reconciles its persisted send-seq against it and refuses to operate if its own value is lower. Fields protected: send-seq, per-bucket receive high-waters, and the grant watermark. When the authority is unreachable the phone stays fail-closed for mutating ops rather than assuming its local value is current. | Test: restore an older state blob, reconnect, and assert the reconciliation refuses to proceed and no seq is reused. The test may not rely on hidden state unavailable after a real rollback. |
...
| S12 Push: trigger, seam rename, FCM sender, preference verb | PB-PUSH-0..3, 5..8 (machine side); **PB-PUSH-9 machine half only** | opus | S0, S8 (PB-PUSH-8 needs a façade method + signed-action work) |
...
made each depend on the other. Every edge above is enumerated, and PB-DOC-7 requires the
manifest be machine-checked so this class of error is caught mechanically rather than by review.
