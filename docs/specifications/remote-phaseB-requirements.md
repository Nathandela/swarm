...
| Biometric freshness | 60 s for input/take_control; **per-use** (`CryptoObject`) for revoke, kill switch, launch, kill | PB-SEC-2 **fence:** android/gate/s20_pbsec2_freshness_test.go |
...
| Biometric-freshness renewal | a typing session crossing the 60 s freshness window must **pause input and re-authorize**, not silently continue or silently drop; the lease itself is not ended by freshness expiry | PB-SEC-2, PB-INPUT-3 **fence:** android/gate/s20_pbsec2_timedprompt_test.go |
