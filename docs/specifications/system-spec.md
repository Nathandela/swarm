...
- **R-5** (Event) WHEN the owner requests a **hands-off handoff** of a source session, swarm SHALL compose the successor's launch **in the daemon** from **pointers only** — the source's canonical native conversation id, its transcript path, its resolved agent working directory, its agent name, and its swarm session id — injecting no transcript content, digest, summary, or filter recipe, and SHALL signal the source in no way (no input, no stop, no kill). The source's status SHALL NOT gate the request: a rate-limited session is indistinguishable from a healthy idle one, so status MAY suggest the method and SHALL NOT select it (ADR-010 Amendment 4). Hands-off sources SHALL be limited to providers with a characterized, anchored history locator: Claude Code and unarchived Codex histories are supported; agy, OpenCode, Hermes, and unknown future providers are not. Codex discovery SHALL be bounded below the daemon user's `~/.codex/sessions` root, SHALL recognize ordinary and reverted-rollout filename forms, SHALL select the newest logical rollout deterministically, and SHALL validate the selected file's first complete `session_meta.payload.id` against the requested stable thread id. Codex histories moved into `~/.codex/archived_sessions`, and a selected rollout available only as `.jsonl.zst`, SHALL refuse by name — never fall back to an older plaintext rollout — until reviewed archive-selection and decompression/export lifecycles exist. A missing or non-canonical conversation identity, an unsupported source agent, an ambiguous history selection, an exhausted scan/read budget, a transcript path that escapes the provider's anchored history root, an unsafe or mismatched file, and a transcript absent at launch time SHALL each refuse **by name** and launch nothing; a refusal SHALL NEVER degrade to a context-free launch. The successor SHALL be linked by `spawned_from` (the parent's local id) with `spawn_intent=handoff` and an empty supervision mode, the source's lifecycle SHALL be untouched, and the composed prompt SHALL state that the source may still be running in the same checkout. WHERE the target CLI differs from the source CLI, the launch SHALL require an explicit owner confirmation naming both CLIs, since the successor is instructed to read a transcript authored under another CLI. This SHALL be a conservative CLI boundary, not a static CLI-to-provider inference: a same-provider cross-CLI route MAY therefore be confirmed, while a same-CLI handoff SHALL require no such confirmation. Hands-off handoff SHALL be owner-tier-only and capability-negotiated so an older daemon cannot silently perform a bare launch.
