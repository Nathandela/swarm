...
- **C-5** (Rollout) Codex telemetry and native compaction lifecycle are observed only for a running app-server version characterized from its initialize response. The independent concurrent-turn and concurrent-manual-compaction gates were run against a live provider on 2026-09-01 and BOTH FAILED (`docs/verification/context-guard.md`): the provider serializes nothing. Automatic dispatch is therefore authorized (ADR-023 amendment 1) ONLY under the daemon's own enforcement — composer-lane write ordering with queue-head and write-boundary revalidation, an effect-window gate refusing daemon-originated typed input from the durable executing record until the outcome is confirmed, held, or latched, unattended sessions only, a bounded confirmation deadline into a no-retry hold, and an exact per-version live-gate allowlist (everything else in a characterized family is observe-only). After a compaction the guard itself dispatched has verifiably completed, the daemon SHALL send at most one ordinary continuation turn, from the composer lane's head, revalidated (unattended, quiet, Stop barrier unchanged, current backend); any failed check, send failure, hold, or crash forfeits it -- never a retry, and never for a compaction the daemon did not write. Claude, OpenCode, AGY, and Hermes remain unavailable until independently characterized.
