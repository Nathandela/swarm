...
| S-3 | Worktree toggle creates `.swarm/worktrees/<name-slug>` + an id-qualified branch for a named session (id suffix on path collision; id-only fallback when unnamed), errors if not a git repo, tears the exact persisted path down on delete | `internal/worktree/worktree_test.go:TestCreateMakesWorktreeAndBranch`, `TestCreateSameNameUsesSessionIDToAvoidCollision`, `TestCreateInNonRepoDirErrors`, `TestRemoveTearsDownWorktree`, `skeleton/agentcwd_test.go:TestAgentCwdWorktreeTeardownStillAnchorsOnTheRepo`, `e2e/worktree_e2e_test.go:TestE2E_Worktree_LaunchRunTeardown` (composite: real named launch → agent's actual cwd is the readable worktree → real delete tears it down, through the assembled daemon) |
...
| P-3 | Latest committed state pushed to subscribers (consecutive changes may coalesce, ADR-008); slow/dead one disconnected via bounded queue, never blocking | `protocol/fanout_test.go:TestFanout_StatusChangeReachesLiveSubscriberWithin1s`, `TestFanout_WedgedSubscriberDisconnectedWithinBound`, `skeleton/renamerace_test.go:TestFanout_RenameNeverRevertedByConcurrentStatus` (converges to latest under commit storm) |
...
| V-5 | Needs-input / Ready-for-review transitions observed in the delivered status stream surface highlight + banner (a state coalesced away before delivery does not banner, ADR-008) | `tui/banner_test.go:TestBanner_OnNeedsInputTransition`, `TestBanner_OnReadyForReviewTransition`, `TestBanner_NotShownOnInitialListing`, `TestBanner_NotShownOnWorkingTransition` |
...
| T-3 | Grid heuristics as fallback, evaluated on output + low-frequency poll | `engine/heuristic_test.go:TestHeuristicConclusiveGrids`, `TestHeuristicInconclusivePreservesCommittedTurn`, `TestNoBusyPoll`, `TestTickReevaluatesStaleHeuristicActive`, `adapter/claude/claude_test.go:TestGridHeuristicFallback_ClassifiesIdlePrompt`, `adapter/codex/codex_test.go:TestGridHeuristicFallback_ClassifiesIdlePrompt` |
| T-4 | No-output-no-CPU beyond threshold while `active` → `unknown`; inconclusive output preserves the committed turn (ADR-007; was inconclusive→`unknown` pre-v0.5) | `engine/staleness_test.go:TestStalenessGuardFlipsActiveToUnknown`, `TestStalenessGuardKeepsBusyActive`, `engine/cpuparse_test.go`, `engine/cpu_integration_test.go:TestSampleCPURealProcesses` |
...
| N-2 | Attach echo <10ms p95 (measured keystroke->PTY write) | `attach/latency_test.go:TestPassthrough_KeystrokeEchoLatencyP95` (E8.5, measures the attach-layer passthrough overhead in isolation — fake session, no network/PTY), **and** the real end-to-end gate: `e2e/latency_gate_test.go:TestE2E_LiveShimKeystrokeEchoLatencyP95` — a real `swarm daemon` + real agent session over a real PTY behind a real `protocol.Client`, timing the keystroke->protocol->daemon->shim->PTY->echo round trip over >=1000 samples, p95 asserted fail-closed against the 10ms production budget in the non-race build (the authoritative N-2 gate; a 250ms sanity ceiling applies only under `-race` so the gate is never skipped). |
