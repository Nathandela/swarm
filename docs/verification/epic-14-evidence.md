...
| E14.4 perf budgets (fail-closed) | N-1 first-paint p95 <100ms @50 with a REAL daemon (`tui/firstpaint_gate_test.go`; the authoritative 100ms assertion is the non-race build, run in a dedicated non-race CI lane; a 2s sanity ceiling is asserted under `-race` so the gate is never skipped). N-2 attach echo p95 <10ms — asserted at BOTH layers: (a) the attach passthrough's own added overhead in isolation (`attach/latency_test.go:TestPassthrough_KeystrokeEchoLatencyP95`; fake session, no network/PTY), and (b) end-to-end over a LIVE shim (`e2e/latency_gate_test.go:TestE2E_LiveShimKeystrokeEchoLatencyP95`) — a real `swarm daemon` + real agent over a real PTY with a real client attached, timing the keystroke->protocol->daemon->shim->PTY->echo round trip over 1000 samples (measured p95 ~0.11ms), fail-closed; like N-1 the authoritative 10ms assertion is the non-race build (needs a dedicated non-race CI lane), and a 250ms sanity ceiling is asserted under `-race` so the gate is never skipped. N-3 spinner-churn collapse (`transcript/transcript_test.go`) + real daemon+shims idle-CPU <5%/core over ≥60s (`daemon/idlecpu_integration_test.go`, `//go:build integration`, run in both CI integration lanes; measured mean 0.000%). |
...
gofmt · `go build ./...` · `go vet ./...` · `GOOS=linux go build ./...` clean. Whole module green under `-race` (28 pkgs) AND non-race (the authoritative first-paint 100ms gate and the live-shim keystroke-echo 10ms gate), plus the `-tags integration` idle-CPU soak, plus all 5 fuzz targets actively fuzzed. Cross-tag builds (`realcli`, `compat_shim_v0/v2`, `integration`) all compile; the shipped untagged build is unchanged.
