...
| M3.2 open policy | **GREEN (model + call site + anchorless read)** | `SessionDetailOpen.plan` keeps the 30 s monotonic throttle. A cold-opened empty transcript now sends `interaction_history` with the explicit empty `before_item` newest-page sentinel, claims the reply silently, and gains its first stable item anchor. Conversation Reload uses the same session-scoped bounded read instead of global journal reseed; the global journal stale verdict remains honest until its separate transport repair completes. `TestConversationReloadUsesBoundedNewestHistory` fences both call paths; the real-envelope worst-case test proves maximal 16 KiB records are trimmed on item boundaries before encryption/base64 can cross relay `MaxFrame`. |
...
ii. ~~**Open a session this phone has never held an item for and see its history.**~~ **CLOSED
   2026-08-30:** empty `before_item` requests the bounded newest retained page. Cold-open claims
   it silently; conversation Reload claims it with visible refusal handling.
...
- ~~**An anchorless "newest page" read** (CANNOT YET (ii)).~~ **CLOSED 2026-08-30** by ADR-014 A11.
