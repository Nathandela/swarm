# PB-SEC-2 — the per-use tier, wired (ADR-007 B51 and B57)
...
| F3 | `AUTH_DEVICE_CREDENTIAL` added to the **key only** — ADR-007 B57 reversed by half | **FAIL** |
...
### 4.3 The startup gate (ADR-007 B57's bill)
...
- **B44's foreground `AUTH_TIMEOUT_EXPIRED` timer** is still not built. Its recorded reason — no
  ...
  standing. The residual now rests on the smaller claim it can carry (ADR-007 B57).
- **A handset with no enrolled Class-3 biometric cannot use revoke, kill or the content tier** until
  ...
  refusing `AUTH_DEVICE_CREDENTIAL`, argued in ADR-007 B57, and both are now told what to do rather
...
- **A pre-existing hole found while checking that claim, and closed.** A PIN-only handset did not reach
  ...
  USER_AUTH_PER_USE is answered from the API level. See §4.3 and ADR-007 B57.
