...
## PB-DOC-2's FIRST HALF IS NOT DONE, AND THE FINAL AUDIT WILL LOOK FOR IT

PB-DOC-2 has two halves: **"Phase B exit criteria in `implementation-goals.md`"**, and "a
verification file maps every PB-* ID to evidence". Checked, not assumed:

- The **second** half is done — `docs/verification/remote-phaseB-traceability.md`, generated by
  `scripts/phaseb-traceability.py`, one row per requirement with owner, status and evidence path.
- The **first** half is **not**. `docs/specifications/implementation-goals.md` contains global goals,
  per-epic definitions of done, and the orchestration protocol — and **no Phase B section at all**.
  `grep` for "Phase B" or "PB-" in that file returns nothing.

This matters more than a missing document usually would. The final audit committee validates
production readiness against **stated exit criteria**, and the file those criteria are supposed to
live in does not yet mention this phase. Without them the committee is left inferring the bar from
143 requirement rows, which is exactly the inference the criteria exist to remove — and a phase can
look complete against its requirements while failing a criterion nobody wrote down.

**Owner: S20**, which owns PB-DOC-2. Its brief already covers the traceability half; this is the half
that is missing, and it should be written BEFORE the committee convenes rather than in response to
it.
