...
| PB-SYNC-4 | NOT DERIVED | B121 recorded READ only. The gate is genuinely capability + kill switch and both are live, but this row's file fences only the capability half while its failure message says "with the kill switch on" and nothing in it engages the kill switch. |
