...
| PB-SYNC-1 | DERIVED | Backfilled from ADR-007 B121 (round-7 derivation, run then, written in prose). Two mutations: `streamsOf` drops `StreamTerminal` from the shared bucket -> 4 failures; `repairedBy` lets a command reply clear `StreamReply` -> `TestB85_AContiguousReplyDoesNotClearTheReplyStream` fails. |
| PB-SYNC-2 | NOT DERIVED | Round-7 FINDING S-1, open: the journal repair channel's only author (`App.Resync`) has no production Kotlin caller, ledgered as the action on a screen that does not exist. After one hole the journal stays stale through 20 journal records, 20 terminal snapshots and a full reconcile; terminal self-heals, journal never does. |
| PB-SYNC-3 | DERIVED | Backfilled from B121: `commitReceive`'s contiguity branch made to clear the channel regardless of contiguity -- the optimistic clear this row forbids -> caught. |
| PB-SYNC-4 | DERIVED | The kill-switch half made unreachable — `if cc.srv.remoteTier && cc.srv.remoteControlDisabled()` in `handleJournalRead` replaced with `if false`, so `journal_read` serves the journal with remote control switched OFF -> `TestProtocol_OffGatesJournalSubscribe` fails, and `TestPhonesim_ObserveTerminalRecoversAfterKillSwitchToggle` in `internal/skeleton` fails with it. Reverted; `server.go` sha256 `282d5f4ab16b...` identical. **This corrects an earlier NOT DERIVED verdict that recorded the kill-switch half as fenced "one file over" and READ rather than mutated.** It is fenced, in two packages, and the second failure is the useful one: the phonesim test drives a real toggle end to end, so the guard is not merely present at the call site but observable in the behaviour a phone sees. |
| PB-SYNC-5 | NOT DERIVED | Conditional row whose antecedent is false -- the shipped resync decision is UNSIGNED -- so B121 recorded it vacuously clean and mutated nothing. |
| PB-SYNC-6 | DERIVED | Backfilled from B121. Two mutations: `App.resyncBudget` returns nil unconditionally -> caught; the drain loops on a non-empty page instead of on cursor progress -> caught. |
| PB-SYNC-8 | DERIVED | Backfilled from B121: `SessionCache.reseed` made to merge instead of replace -- the exact PB-SYNC-8 defect -> 2 failures. |
