...
| PB-TIME-1 | DERIVED | `internal/skeleton/deviceauth.go`'s expiry guard neutered -- `if now.After(cmd.ExpiresAt)` replaced with `if false`, so no signed command ever expires -> **4 failures**: `TestPolicy_ExpiredCommandRejected` (the direct one), `TestPBPUSH8_ForgedOrExpiredPushPrefsIsRefused`, and both `TestB42PBNET4_*` tests, which depend on the 1-minute TTL being what makes a queued command undeliverable. Reverted in the same command; `deviceauth.go` sha256 `00d27a109d25...` identical before and after. |
| PB-TIME-2 | NOT DERIVED | **One clause of a quantified row was mutated; that is not deriving it.** `internal/protocol/server.go`'s server-side cap neutered -- `if c.ExpiresAt.After(cc.srv.now().Add(maxCommandValidity))` replaced with `if false` -> `TestProtocol_CommandExpiresAtCapped` fails, and nothing else in `protocol` or `skeleton` moves. So the command-expiry clause IS fenced. But this row's subject is **every** security-relevant timestamp -- it names envelope `IssuedAt` and the PB-GW-2 bounded-age check that consumes it, and push expiry/replay -- and those were not mutated. Reverted; `server.go` sha256 `282d5f4ab16b...` identical. Deriving this row needs one mutation per named timestamp, and the enumeration of "every" needs checking against the fences first. |
| PB-TIME-3 | DERIVED, with a finding | `SkewMonitor.Check()` made to `return nil` instead of `m.lastErr`, so skew is never reported -> `TestS11Skew_CheckMirrorsTheLastMeasurement` and `TestS11Skew_OfflineIsNotSkewed` fail. Reverted; `skew.go` sha256 `ae73040c063f...` identical. **THE FINDING: `mobile` passes under that mutation.** `Check()` is called in production at `mobile/relay.go:726` and `Sent()` at `mobile/commands.go:735`, but nothing fences that the PHONE ACTS on the verdict -- the fence's subject is the monitor, the requirement's subject is a detection protocol the phone participates in. Same dropped-quantifier shape found at six other rows. |
