...
| PB-RUN-2 | NOT DERIVED | `CAMERA` removed from `AndroidManifest.xml` -> survives the Go gate, which shows only that the Go gate does not cover it. The Kotlin fences (`PermissionGateTest`, `RuntimeManifestTest`) were read and the resolver's production reachability confirmed, but neither was ever made to fail: the app module did not compile |
