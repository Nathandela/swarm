...
| Row | Verdict | The mutation |
|---|---|---|
| PB-TOOL-1 | **DERIVED — clean** | gomobile pin diverged from `go.mod`'s require → caught; `SWARM_AAR_ABIS` dropped `arm64-v8a` → caught; `-androidapi` vs `minSdk` conflation separately fenced |
| PB-TOOL-2 | **DERIVED — clean at the pin** | ABI-set mutation caught. The artifact inspection itself is real but sits in the tagged lane, whose invocation was finding A |
| PB-TOOL-3 | **DERIVED — clean** | release signing repointed at a repo-relative keystore → caught; no-keystore-tracked, no-password-tracked and gitignore fences all present |
| PB-TOOL-4 | **FINDING C — fixed** | wrapper `distributionSha256Sum` swapped for another valid 64-hex value → SURVIVED; `SWARM_GRADLE_DISTRIBUTION_SHA256` zeroed → SURVIVED |
| PB-TOOL-5 | **DERIVED — clean** | fenced by CI-job inspection plus the real Go suite |
| PB-TOOL-6 | **FINDING B — fixed** | `test` dropped from `./gradlew --no-daemon lint test` → SURVIVED (`lint` dropped → caught) |
| PB-TOOL-7 | **FINDING A — fixed** | `continue-on-error: true` on the Gradle-gate step → SURVIVED; `if: false` on the tagged-artifact step → SURVIVED |
| PB-RUN-1 | **DERIVED — clean** | `SWARM_ANDROID_MIN_SDK` 33 → 31 with the matrix unchanged → caught; matrix contiguity, endpoint binding and the no-competing-literals check all discriminate |
| PB-RUN-2 | **DERIVED — clean (resolver) / NOT DERIVED (manifest half)** | resolver reachable from two production call sites; `CAMERA` removed from `AndroidManifest.xml` → survives the Go gate, so that assertion lives **exclusively** behind the CI Gradle step |
| PB-RUN-3 | **FINDING D — open** | fenced twice at a *description* and never at the socket (below) |
| PB-RUN-4 | **DERIVED — clean** | `wake high` → `normal` → caught; joined to the connectivity table in both directions |
| PB-RUN-5 | **NOT DERIVED** | its behavioural fence is Kotlin and could not be mutated (below) |
