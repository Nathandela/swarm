...
## Independent derivation, 2026-07-31 (round 8) — what was verified, and what could not be

S13's thirteen rows had read MET since 2026-07-25 and had **never been independently derived**.
This section is that derivation. The method was: for each row, name its fence, name the symbol
that fence drives, confirm the symbol is reachable from production, and **mutate it** — a fence
that was not broken is a fence that was not verified. Every mutation ran in a detached worktree,
since removed.

**Read the verdict column, not the row's MET status.** "We looked and it held" and "we could not
look" are different facts and the traceability table cannot tell them apart — it is generated by
`scripts/phaseb-traceability.py` and its status column is about shipping, not about derivation.

## Derivation

**MACHINE-READABLE. `scripts/phaseb-traceability.py` reads this section** to emit the
traceability table's DERIVATION column, and `internal/verify` fences that it does. The shape is
fixed: one row per requirement, the verdict token `DERIVED` or `NOT DERIVED`, and — for
`DERIVED` — **the mutation that was made to fail, in the same row**. A `DERIVED` verdict with an
empty mutation cell is refused and counted as NOT DERIVED; the token cannot be claimed without
naming what was broken. Any requirement with no row here is NOT DERIVED.

**`DERIVED` means somebody made this row's fence fail on purpose and restored it.** Not that a
test exists, not that the slice shipped. `NOT DERIVED` is the default and is **a statement about
the audit, not about the code** — it means nobody has looked, and it is orthogonal to whether the
row is shipped or met.

The definition is applied strictly here, which moved two rows out of "clean": PB-TOOL-5's
mutation would not apply, and PB-RUN-2's resolver fence was read and its reachability confirmed
but never broken. Reading a fence is not deriving it.

| Requirement | Verdict | The mutation, and its result |
|---|---|---|
| PB-TOOL-1 | DERIVED | gomobile pin diverged from `go.mod`'s require -> caught; `SWARM_AAR_ABIS` dropped `arm64-v8a` -> caught |
| PB-TOOL-2 | DERIVED | `SWARM_AAR_ABIS` dropped its named ABI -> caught. The artifact inspection itself is real but sits in the tagged lane, whose invocation was finding A |
| PB-TOOL-3 | DERIVED | release signing repointed at a repo-relative keystore -> caught |
| PB-TOOL-4 | DERIVED | wrapper `distributionSha256Sum` swapped for another valid 64-hex value -> SURVIVED; `SWARM_GRADLE_DISTRIBUTION_SHA256` zeroed -> SURVIVED. **Finding C, fixed**; both now caught |
| PB-TOOL-5 | NOT DERIVED | the intended mutation (removing `go build ./...` from the CI test job) would not apply, and no other was attempted. Reading its fences is not deriving them |
| PB-TOOL-6 | DERIVED | `test` dropped from `./gradlew --no-daemon lint test` -> SURVIVED (`lint` dropped -> caught). **Finding B, fixed**; now caught |
| PB-TOOL-7 | DERIVED | `continue-on-error: true` on the Gradle-gate step -> SURVIVED; `if: false` on the tagged-artifact step -> SURVIVED. **Finding A, fixed**; both now caught |
| PB-RUN-1 | DERIVED | `SWARM_ANDROID_MIN_SDK` 33 -> 31 with the matrix unchanged -> caught |
| PB-RUN-2 | DERIVED | **Re-derived 2026-07-31 once the Android toolchain ran.** The earlier verdict was NOT DERIVED because the app module did not compile, so no Kotlin fence could be mutated -- an honest limit of the auditor, not of the fence. With the AAR rebuilt and `gradlew` green, the same mutation was run properly: `CAMERA` removed from `AndroidManifest.xml` -> `RuntimeManifestTest > manifest_declares_every_permission_the_gate_resolves FAILED` (`RuntimeManifestTest.kt:40`), `BUILD FAILED in 53s`. Reverted; manifest sha256 `d604140f8f4a...` identical. **The Go gate does survive that mutation** -- the fence is Kotlin-side, which is exactly why the toolchain run was a prerequisite and not a formality. |
| PB-RUN-3 | NOT DERIVED | **Finding D, open.** There is no fence over the row's subject to break: the TSV and the Kotlin object are fenced against each other, and `PhoneSurface.release()` — the code that closes the socket — has no test. Established by inspection, not mutation |
| PB-RUN-4 | DERIVED | `wake high` -> `normal` -> caught |
| PB-RUN-5 | DERIVED | **Re-derived 2026-07-31 once the Android toolchain ran**, for the same reason as PB-RUN-2: the earlier verdict recorded an auditor limit, not a fence limit. `LifecycleConvergence.planFor`'s `NETWORK_LOST` arm mutated `cancelOutstandingWait = true` -> `false` -> `LifecycleConvergenceTest > losing_the_network_cancels_the_outstanding_wait FAILED`, `BUILD FAILED in 56s`. Reverted; sha256 `d8b437da920b...` identical. **A first attempt is recorded as NOT a proof:** replacing the whole arm introduced a non-existent enum member and produced `compileDebugKotlin FAILED` -- a COMPILE failure, which fails tautologically and proves nothing about the fence (ADR-007 B113's neighbour). It was redone as a single field flip that compiles, so the failure is the behaviour and not the syntax. |

## What the derivation found

### FINDING D — PB-RUN-3 is fenced twice at a description, never at the socket. OPEN.

The row's subject is *"what the socket does on backgrounding, Doze, App Standby, battery saver"*.
`connectivity_test.go` fences `android/connectivity-policy.tsv` for totality, closed vocabulary
and joint consistency — genuinely good work. `ConnectivityPolicyTest.kt` fences that the
`ConnectivityPolicy` object **equals that table**. Two representations of one table, carefully
cross-checked against each other.

**Nothing fences that the shipping code obeys the object.** `PhoneSurface.release()` is what
actually closes the socket and no test in the tree exercises it; the only test files mentioning
`release`/`stopObserving` are about the event listener. This is an inspection result, not a
mutation result, and is labelled so: the Kotlin mutation could not be run because the app module
did not compile (below).

### Two rows NOT DERIVED, named rather than counted

`LifecycleConvergenceTest.kt` (PB-RUN-5's behavioural half) and `RuntimeManifestTest.kt`
(PB-RUN-2's Android-runtime half) are Kotlin. They could not be mutated because **the app module
did not compile at `8dfd077`**: commit `692ca66` replaced the `LifecycleConvergence` import in
`PhoneSurface.kt` with duplicates of two imports eighteen lines above, and the file references
`LifecycleConvergence` at line 495 from a different package. Fixed in `cb77823`. **The whole
tagged `androidgate` lane is likewise underived** — it needs a full toolchain run on a quiet host.

That break is the same risk findings A and B describe, realised by accident on the pushed branch
within hours of being measured: `go build`, `go vet` and `go test ./...` are all green over it,
because none of them compiles Kotlin, and the CI Gradle gate that would have caught it is exactly
the step those two findings could silently remove.

### PB-RUN-2 is the best-built row in the tranche, and that is worth recording

Eight rounds of this record are defects; a row built right deserves its line.
`PermissionGateTest.kt` is a truth table over four inputs that includes **the two states real
apps get wrong** — "permanently denied is not `!shouldShowRequestPermissionRationale`", because
that predicate is also false before the first ask, and "POST_NOTIFICATIONS does not exist below
API 33", which is a third answer rather than a convenience. It adds a totality check over the
enum so a permission added by a later slice cannot silently resolve to granted, asserts the
per-permission degraded capabilities are **distinct**, and the "have we asked" bit the platform
does not offer is persisted in SharedPreferences under a data root excluded from both cloud
backup and device-to-device transfer. Reachable from `PairingSurface` and `SettingsSurface`.

### Accepted residual from this derivation

**Which hosts a Gradle distribution may be fetched from is undecided.** The PB-TOOL-4 fix binds
the two copies of the distribution hash to each other; it binds neither to the real Gradle
distribution, and nothing constrains `distributionUrl`'s host — repointing it while keeping the
version and the `https://` scheme survives every fence in the package. That is a policy nobody
has stated, recorded here in PB-APP-11's form rather than invented. See ADR-007 B128.
