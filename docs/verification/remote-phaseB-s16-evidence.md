...
|---|---|---|
| PB-PAIR-5 | NOT DERIVED | **The mutation I ran cannot decide this row, and that is the record.** `pairCancelled` collapsed to the same string as `pairFailed`, so two terminal states become indistinguishable -> `mobile` and `mobile/conformance` **both pass** (13.8s / 180.3s). But that is a B113-shaped non-result: I moved the CONSTANT, and any assertion written as `state != pairCancelled` compares the constant to itself and cannot see it. **Separately, and this is the part worth acting on:** `TestPBPAIR5_EveryTerminalStateIsExplicitAndDistinct` (`s16_pairing_test.go:405`) mentions `cancelled` **only in a comment** — `grep` finds no literal `"cancelled"` assertion in the file. So the distinctness of that state is not pinned by a literal anywhere. Deriving this row needs the mutation at a CALL SITE (make one terminal path set another's state) rather than at the constant. Reverted; `mobile/pairing.go` sha256 `6c23e0c9767e...` identical. |
