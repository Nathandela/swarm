...
| PB-BIND-4 | S8 | **NOT MET** | THE FENCE'S SUBJECT IS KeyCustody; B8'S SUBJECT IS THE BINDING. B8 says key material crosses the binding ONCE and inbound. A SECOND reverse-bound key-crossing interface passes both custody fences: S14's count is written for one owner name, and the walk sees funcs and methods but not interface methods, which its own comment states. They fence DIRECTION; nothing fences COUNT ACROSS TYPES. Only the golden catches it, and this project's own words about that file are 'a review step someone can approve, not a rule that refuses'. NOT VIOLATED TODAY (ADR-007 B116) |
