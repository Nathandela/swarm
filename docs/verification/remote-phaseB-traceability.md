...
| PB-SEC-3 | S18 | **NOT MET** | the automated log scan is FAIL-OPEN: stripLineComments truncates at the first // and is not string-literal-aware, so a URL in a log message hides the arguments after it -- measured with a shipped function writing plaintext AND the content key while the whole gate stayed green. Its evidence artifact is generated by the same broken scan, so the artifact certifies the blindness (ADR-007 B72) |
