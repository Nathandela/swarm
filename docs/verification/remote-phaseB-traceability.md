...
| PB-NET-7 | S6 | **NOT MET** | A LIVE DEFECT, not a documentation gap -- and the reassurance that hid it was MINE. B109 said MailboxWait 'bypasses the bound by construction under the relay's own 25 s ceiling'. THAT CEILING IS ENFORCED SERVER-SIDE, BY THE DECLARED ADVERSARY. Probed against a relay that answers nothing: MailboxWait(context.Background()) STILL PARKED AFTER 70s -- 2.8x the ceiling. mailboxWait never calls bounded(); the gateway passes an undeadlined ctx (remotegw/command_loop.go); cmd/swarm-remote dials ONCE and holds that client for the process lifetime with no redial loop; and the package has NO ping and NO read deadline, so c.done never closes against a silent-but-alive peer. The gateway's command-IN loop therefore parks for the life of the process -- keystrokes, take_control and KILL stop being processed, while the phone still reads online because its appends succeed into the relay's store. This is B94(1)'s CRITICAL on the MACHINE hop, reachable benignly by a half-open TCP. Every OTHER named clause IS fenced and mutation-proven. Fix in flight (ADR-007 B112) |
