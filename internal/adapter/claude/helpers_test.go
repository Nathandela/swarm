...
// Shared test helpers for the claude adapter suite: pure argv/JSON inspection, the
// vt grid renderer (the exact projection the engine hands an adapter), the T-5
// import-boundary probe (go list -deps), and the E9.2 no-IO source scan. Mirrors
// the reference adapter's harness so the two stay consistent.
...
// settingsValue returns the value of the --settings flag in argv, supporting both
// the separate-arg form ("--settings", "<json>") and the joined form
// ("--settings=<json>"). ok is false when the flag is absent.
func settingsValue(argv []string) (string, bool) {
	for i, a := range argv {
		if a == "--settings" {
			if i+1 < len(argv) {
		...
		if v, found := strings.CutPrefix(a, "--settings="); found {
			return v, true
		}
	}
	return "", false
