...
// Shared test helpers for the opencode adapter suite: pure argv inspection, the
// vt grid renderer, the T-5 import-boundary probe, and the E9.2 no-IO source
// scan. Mirrors the claude/codex adapter harnesses so all three stay consistent.
...
// decoded snapshot — the projection the engine hands an adapter at runtime.
...
// exists yet, so it flips from skip to a real assertion the moment the adapter
// is implemented.
