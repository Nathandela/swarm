...
func TestDeriveDims(t *testing.T) {
	...
	cases := []struct {
		...
		{"notification with no subtype degrades to none (B5, not a false permission)", "Notification", nil,
			map[string]string{PayloadKeyTurn: "idle", PayloadKeyInteraction: "none"}},
		{"notification idle subtype -> none", "Notification", map[string]string{"notification_type": "idle"},
			map[string]string{PayloadKeyTurn: "idle", PayloadKeyInteraction: "none"}},
