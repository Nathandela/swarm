...
// busy-poll; inconclusive detection maps deterministically to turn=unknown.
//
// PIN: the fixtures here are CLI-AGNOSTIC — they exercise the engine MECHANISM
// (evaluate-on-output, re-evaluate-on-poll, precedence, staleness, unknown on
// inconclusive), not any Claude/Codex-specific rule. Per-CLI grid rules are Epic
// 11 adapter work. The generic patterns the engine must recognize:
//   - a settled trailing prompt sentinel with a parked cursor -> idle/none
//   - a trailing braille/ASCII spinner glyph -> active
//   - anything else (prose, blank) -> unknown (never a confident guess)
...
// E10.8 (T-4): inconclusive detection maps DETERMINISTICALLY to unknown. Drive a
// definite turn first, then feed an inconclusive grid and assert it downgrades to
// unknown — repeatably, with no flip-flop.
func TestHeuristicInconclusiveMapsToUnknown(t *testing.T) {
	cases := []struct {
		name string
		snap *vt.Snap
	}{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Establish a definite turn so the inconclusive read is an observable change.
			...
			e.OnOutput("s1", tc.snap)
			got, ok := rec.last()
			if !ok || got.s.Turn != status.TurnUnknown {
				t.Fatalf("%s grid -> turn=%s (emitted=%v), want unknown (T-4)", tc.name, got.s.Turn, ok)
			}

			// Re-evaluating the same inconclusive grid is stable (deterministic).
			e.OnOutput("s1", tc.snap)
			if got, _ := rec.last(); got.s.Turn != status.TurnUnknown {
				t.Fatalf("%s grid second eval -> turn=%s, want stable unknown", tc.name, got.s.Turn)
			}
		})
	}
}
