...
func s15State() State {
	st := State{
		...
		DiscardRecoveryIncarnation: s15RelayIncarnation,
		DiscardRecoveryCursor:      s15RelayCursor + 1,
...
func s15Inventory() []s15Tier {
	...
	return []s15Tier{
		...
		{field: "DiscardRecoveryIncarnation", needles: s15Str(s15RelayIncarnation),
			why: "the exact pre-discard mailbox identity; it is the crash-safe phase boundary that prevents " +
				"a retry from deleting the replacement mailbox"},
		{field: "DiscardRecoveryCursor", needles: s15Num(s15RelayCursor+1, 8),
			why: "the exact retained head cutoff authorized for a crash-safe discard retry"},
