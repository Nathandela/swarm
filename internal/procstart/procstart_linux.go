...
// StartTime returns a stable, monotonic-per-process identity token for
...
func StartTime(pid int) (int64, error) {
	...
	if rparen < 0 || rparen+2 > len(s) {
		return 0, fmt.Errorf("procstart: malformed /proc/%d/stat", pid)
	}
	...
	if len(fields) <= starttimeIdx {
		return 0, fmt.Errorf("procstart: /proc/%d/stat has too few fields", pid)
	}
	v, err := strconv.ParseInt(fields[starttimeIdx], 10, 64)
	if err != nil {
		return 0, fmt.Errorf("procstart: parse starttime: %w", err)
	}
	return v, nil
}
