...
func TestDialRejectsMachineProbePurposeBeforeNetwork(t *testing.T) {
	pub, _, err := ed25519.GenerateKey(nil)
	if err != nil {
		t.Fatal(err)
	}
	_, err = Dial(context.Background(), Profile{
		RelayURL: "wss://127.0.0.1:1", MachineRID: strings.Repeat("0", 32), OperatorNamespace: "owner",
	}, Auth{PublicKey: pub, Sign: func([]byte) ([]byte, error) { return make([]byte, ed25519.SignatureSize), nil }, Role: RoleMachine, Purpose: PurposeProbe})
	if err == nil || !strings.Contains(err.Error(), "invalid purpose") {
		t.Fatalf("Dial error = %v, want machine probe-purpose refusal", err)
	}
}
