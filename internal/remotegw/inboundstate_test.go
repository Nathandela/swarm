...
func TestInboundState_FutureSchemaFailsClosed(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"schema_version":3,"machine":"machine-1","cursor":3,"streams":[]}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := OpenInboundState(path, "machine-1"); !errors.Is(err, errCorruptInboundState) {
		t.Fatalf("OpenInboundState(v3) = %v, want fail-closed corrupt-state error", err)
	}
}

func TestInboundState_V2MalformedIncarnationFailsClosed(t *testing.T) {
	path := filepath.Join(t.TempDir(), "malformed-incarnation.json")
	body := `{"schema_version":2,"machine":"machine-1","cursor":3,"mailbox_incarnation":"NOT-CANONICAL","streams":[]}`
	if err := os.WriteFile(path, []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := OpenInboundState(path, "machine-1"); !errors.Is(err, errCorruptInboundState) {
		t.Fatalf("OpenInboundState(malformed v2 incarnation) = %v, want fail closed", err)
	}
}
...
func TestInboundState_RewindCursorPreservesReplayHighWaters(t *testing.T) {
	path := filepath.Join(t.TempDir(), "inbound.json")
	state, err := OpenInboundState(path, "machine-a")
	if err != nil {
		t.Fatalf("OpenInboundState: %v", err)
	}
	stream := InboundStream{Sender: [8]byte{1, 2, 3}, Epoch: 7}
	...
	if got := state.Load(); got.Cursor != 0 || got.Highest[stream] != 11 {
		t.Fatalf("after rewind = %+v, want cursor 0 with replay high-water 11", got)
	}

	reopened, err := OpenInboundState(path, "machine-a")
	if err != nil {
		t.Fatalf("reopen: %v", err)
	}
	if got := reopened.Load(); got.Cursor != 0 || got.Highest[stream] != 11 {
		t.Fatalf("durable rewind = %+v, want cursor 0 with replay high-water 11", got)
	}
}
