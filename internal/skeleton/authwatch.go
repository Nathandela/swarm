...
func (w *authWatcher) recycle(agent string, m persist.Meta) (retry bool) {
	...
	// Shrink the defer gate's check-to-kill window to one refetch: a prompt may
	// have started since workPending classified this row.
	if cur, ok := w.get(local); !ok || cur.Status.Process != status.ProcessRunning ||
	...
	if err := w.kill(local); err != nil {
		delete(w.state.Killed, local)
		if serr := w.saveState(); serr != nil {
			log.Printf("authwatch: persist claim release for %s: %v", local, serr)
		}
