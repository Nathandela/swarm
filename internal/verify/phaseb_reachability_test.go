...
var b94Allowed = map[string]string{
	...
	// direction it was pointing.
	//
	// NewPairingWakeKey's row is GONE because pairing now mints and conveys the wake key
	// through the negotiated push-binding capability (mobile/pushgateway.go). The
	// bidirectional fence caught the stale parked-gate exemption when that production path
	// landed.
	...
	// ---- Mirror M1.1's temporary row is GONE, collected by the bidirectional arm --------------
	// claude.RecognizePermissionDialog was allowlisted for exactly one wave, "recorded ahead of
	// its caller by design", with the removal condition written into the row itself. M1.2 wired
	// the call (internal/adapter/claude.ApprovalKeys -> internal/skeleton's approve path), this
	// test failed with STALE EXEMPTION on its first run afterwards, and the row was deleted.
	// Left here as the record that the instrument collected its own debt.
