...
NOT_MET = {
    ...
    "PB-BIND-3": "THE TEST NAMED 'Every' ENUMERATES NOTHING. "
                 ...
                 "properly error-classed and traced, passed BOTH full suites (mobile 16.1s, "
                 "conformance 207.1s). The structural half is genuinely strong and "
                 "bidirectional, so a new method cannot go UNTRACED -- it can go "
                 "UNEXERCISED (ADR-007 B116)",
    "PB-BIND-4": "THE FENCE'S SUBJECT IS KeyCustody; B8'S SUBJECT IS THE BINDING. B8 says "
                 "key material crosses the binding ONCE and inbound. Adding a SECOND "
                 "reverse-bound key-crossing interface passes both custody fences -- S14's "
                 "count is written `if owner == \"KeyCustody\"`, and PB-BIND-4's walk sees "
                 "funcs and methods but not interface methods, which its own comment states. "
                 "The gap they close is DIRECTION; the gap nobody closes is COUNT ACROSS "
                 "TYPES. Only the golden catches it, and the golden is 'a review step "
                 "someone can approve, not a rule that refuses' -- that project's own words, "
                 "about this same file. NOT VIOLATED TODAY (ADR-007 B116)",
    "PB-BIND-6": "'DROP-OLDEST' IS STATED IN THREE PLACES AND ENFORCED BY NONE. Section "
                 "6.0's budget row is '256 items, drop-oldest with a surfaced overflow "
                 "signal'. Inverting the policy to drop-NEWEST -- same constant, same "
                 "counter, same doc string -- leaves BOTH fences passing: the source fence "
                 "checks the constant and the prose, the runtime fence checks that the core "
                 "did not stall and that SOME events dropped. Nothing checks WHICH. Not "
                 "cosmetic: under drop-newest a slow UI discards the CURRENT state and keeps "
                 "the stale one, so the connection plane renders an old state and never "
                 "converges. IMPLEMENTED CORRECTLY TODAY (events.go drops the head), "
                 "unfenced against its own inversion (ADR-007 B116)",
