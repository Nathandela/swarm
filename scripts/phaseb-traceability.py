...
NOT_MET = {
    "PB-NET-7": "EVERY NAMED CLAUSE IS NOW FENCED OVER LIVE CODE, and ONE QUANTIFIER "
                "RESIDUAL keeps it red pending round 7 -- flagged for the committee "
                "rather than adjudicated here, because closing rows on my own partial "
                "reads is how four of this round's nine false rows happened. Bounded at "
                "the committee's 10 s, pinned by a test transcribing the value from "
                "section 6.0's table rather than from the constant (2ae1386). "
                "Cancellation honoured on BOTH the generic request and the dial, restored "
                "after the dead-package deletion took them -- mutation shows a 300 ms "
                "context taking 30.009 s without it (056f1af). No goroutine leak across "
                "12 connect/disconnect cycles, the only such assertion in the remote "
                "stack (2ae1386). Calls after close: typed refusal on both arms, plus "
                "Close idempotency, which a shared fixture had been exercising on every "
                "run while nothing named it (2727f0d). THE RESIDUAL: the row says "
                "'timeouts EVERYWHERE' and nothing ENUMERATES the call paths -- the bound "
                "sits on roundtrip, which every non-wait call takes, and MailboxWait "
                "bypasses it by construction under the relay's own 25 s ceiling. Sound by "
                "ARGUMENT, unfenced -- residual 4.23's shape, and the committee has "
                "refused that argument twice this round (ADR-007 B99, B105, B109)",
